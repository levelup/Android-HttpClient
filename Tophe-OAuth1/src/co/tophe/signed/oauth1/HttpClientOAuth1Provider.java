...
public class HttpClientOAuth1Provider {
	...
	private static final BaseResponseHandler<InputStream> OAUTH1_RESPONSE_HANDLER = new BaseResponseHandler<InputStream>(XferTransformResponseInputStream.INSTANCE);
	 ...
	 * <p>The {@code requestFactory} can do special processing on the response via {@link co.tophe.BaseHttpRequest#setResponse(co.tophe.HttpResponse) HttpRequest.setResponse}
	 * 
	...
	public HttpClientOAuth1Provider(HttpClientOAuth1Consumer consumer, String requestTokenUrl, String accessTokenUrl, String authorizationWebsiteUrl) {
		...
		this.provider = new DefaultOAuthProvider(requestTokenUrl, accessTokenUrl, authorizationWebsiteUrl) {
			...
			protected HttpRequest createRequest(String endpointUrl) throws IOException {
				...
				return new OAuth1RequestAdapter(new HttpEngine.Builder<InputStream,ServerException>()
						.setRequest(request)
			...
			protected HttpResponse sendRequest(HttpRequest request) throws IOException {
				...
				HttpEngine<InputStream,ServerException> processEngine = requestAdapter.unwrap();
				try {
